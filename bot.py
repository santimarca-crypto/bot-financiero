...
async def cancelar_cmd(u: Update, ctx):
    if not ctx.args:
        await u.message.reply_text("Uso: /cancelar N\nEj: /cancelar 3\n(el numero lo ves en /historial como #3)")
        return
    try:
        n = int(ctx.args[0])
    except:
        await u.message.reply_text("Pon un numero. Ej: /cancelar 3")
        return
    hoy = datetime.now().strftime("%d/%m/%Y")
    with db() as c:
        rows = c.execute("SELECT * FROM ops WHERE fecha=? ORDER BY id", (hoy,)).fetchall()
        if n < 1 or n > len(rows):
            await u.message.reply_text("No existe la operacion #" + str(n) + " de hoy.")
            return
        r = rows[n - 1]
        op_id = r["id"]
        e = r["tipo"].upper()
...
async def corregir_cmd(u: Update, ctx):
    if len(ctx.args) < 3:
        await u.message.reply_text("Uso: /corregir N USD TC\nEj: /corregir 3 2000 1380\n(el numero lo ves en /historial como #3)")
        return
    try:
        n = int(ctx.args[0])
